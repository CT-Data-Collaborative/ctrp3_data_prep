...
def long_date_string_to_datetime(value):
    DATE_FORMATS = ["%m/%d/%y %H:%M", "%m/%d/%Y %H:%M", "%m-%d-%y", "%m/%d/%y", "%m/%d/%Y"]
    parsed_date = None
    last_error = None
    for data_format in DATE_FORMATS:
        try:
            parsed_date = eastern.localize(datetime.datetime.strptime(value, data_format))
...
def test():
    ...
    c.build_schema()
    c.clean('loading_v1.csv', True, True)
    return c
